# Guide: Running the Regression Test Pipeline

This guide covers the operational details of running trace-tools regression tests. For an overview of how tracing and observability work, see the [README](README.md). For how to make your app observable, see [GUIDE-APP-DEVELOPER.md](GUIDE-APP-DEVELOPER.md).
...
Open the app with the XMLUI inspector, click through the journey yourself, export the trace JSON, and save it as a baseline. This is the standard workflow — it requires no tooling beyond the app itself and works for anyone.
...
The AI generates a spec, runs it, and the captured trace becomes the baseline. The spec is disposable scaffolding — the baseline is what matters. This is useful for rapidly building out a test suite without manually performing each journey.
...
- **API rowcount transitions** — when a baseline includes `apiResult` entries with `type: "rowcount"`, the generator asserts the *direction of change* between steps rather than exact counts. The first time an endpoint appears, it asserts `toBeGreaterThan(0)` (non-empty) plus a shape check on the response keys. On subsequent occurrences of the same endpoint, it compares the baseline counts and asserts `toBeGreaterThan`, `toBeLessThan`, or `toBe` relative to the previous capture variable. This mirrors the `up`/`down`/`same` vocabulary used by `appTraceShapes` in semantic comparison, and keeps generated tests stable across environments where absolute row counts differ.
...
cp trace-tools/xmlui-parser.es.js public/xmlui-parser.es.js
```
...
- **Don't worry about being clean.** Extra clicks, hesitations, and accidental interactions are fine. The initial capture just needs to be functionally correct — hitting the right APIs, submitting the right forms, navigating the right pages.
...
### App-level accessibility fixes

Sometimes the generated test fails not because of an engine or trace-tools gap, but because the app itself has duplicate accessible names. Playwright's strict mode requires selectors to match exactly one element — `getByRole('button', { name: 'Regenerate' })` fails when three buttons all say "Regenerate".

The fix is in the app, not the engine or trace-tools. For example, `core-ssh-server-ui` had three identical "Regenerate" buttons for RSA, ECDSA, and Ed25519 host keys. Changing the label from `"Regenerate"` to `"{'Regenerate ' + $props.algorithm}"` gave each button a unique accessible name — `Regenerate RSA`, `Regenerate ECDSA`, `Regenerate Ed25519` — which fixes both the test selector and the screen reader experience.

Similarly, if a modal's X icon button and a text "Close" button both have `aria-label="Close"`, the selector is ambiguous. That specific case was fixed in the engine (X button → `aria-label="Close dialog"`), but similar collisions in app code should be fixed at the app level by giving each button a distinct label.

**Rule of thumb:** if Playwright reports a strict mode violation (multiple elements match), check whether the app's labels are genuinely distinct. If not, fix the app — it's an accessibility improvement, not a test hack.
...
**Important:** `xs-diff.html` imports `xmlui-parser.es.js` (a standalone XMLUI parser module) via `<script type="module">`. Both files must be in the app's `xmlui/` directory (the `<Inspector />` component loads `xmlui/xs-diff.html`). If `xmlui-parser.es.js` is missing, the inspector will silently fail to initialize — the iframe loads and shows buttons, but the module script never executes and no trace data is displayed. Copy both from trace-tools:
...
- **Mixed form control types.** The formData fill logic targets `getByRole('textbox')`. Forms with textarea, spinbox, or other non-textbox input types will have fewer fillable elements than formData fields. The generator handles this gracefully when textbox interactions exist in the trace (it fills by label match), but the blind nth()-based fallback for forms with no textbox interactions may fill wrong fields. Forms with mixed control types work best when captured with actual keystrokes rather than `.fill()`.

- **Tree child caching.** After moving or pasting items into a folder, the tree sidebar may not show the new children until the folder is collapsed and re-expanded — and even then, the tree may serve cached children. Tree verification steps in generated tests skip gracefully when expected treeitems aren't found, but this means some post-mutation tree assertions are effectively no-ops.
